...
public interface DetectionHistoryRepository extends JpaRepository<DetectionHistory, Long>,
        JpaSpecificationExecutor<DetectionHistory> {
    ...
    @Query("SELECT d FROM DetectionHistory d WHERE d.vehicle.id = :vehicleId ORDER BY d.createdAt DESC")
    List<DetectionHistory> findLatestDetectionsByVehicle(@Param("vehicleId") Long vehicleId);

    @Query("SELECT d FROM DetectionHistory d WHERE d.complianceStatus = 'NON_COMPLIANT' ORDER BY d.createdAt DESC")
