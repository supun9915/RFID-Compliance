...
public interface DocumentRepository extends JpaRepository<Document, Long> {
    ...
    List<Document> findExpiredDocuments(@Param("vehicleId") Long vehicleId, @Param("date") LocalDateTime date);

    @Query("SELECT d FROM Document d WHERE d.vehicle.id = :vehicleId AND d.endDate >= :date")
    List<Document> findValidDocuments(@Param("vehicleId") Long vehicleId, @Param("date") LocalDateTime date);
